...
Std_ReturnType gpio_pin_direction_init(const pin_config_t * _pin_config)
{
    ...
    if(((void*)0) == _pin_config || _pin_config -> port > 5 - 1 || _pin_config -> pin > 8 - 1)
    {
...
{
    Std_ReturnType ret = (Std_ReturnType)0x01;
    if(((void*)0) == _pin_config || ((void*)0) == dic_status || _pin_config -> port > 5 - 1 || _pin_config -> pin > 8 - 1)
    {
        ret = (Std_ReturnType)0x00;
    }
    else
    {
        *dic_status = ( (*lat_registers[_pin_config -> port] >> _pin_config -> pin) & (uint8)1 );
    }
    return ret;
}
...
Std_ReturnType gpio_pin_read_logic(const pin_config_t * _pin_config, logic_t *_logic)
{
    ...
    {
        *_logic = ( (*port_registers[_pin_config -> port] >> _pin_config -> pin) & (uint8)1 );
    }
    return ret;
}
...
# 248 "Mcal_Layer/GPIO/hal_gpio.c"
Std_ReturnType gpio_port_toggle_logic(port_index_t port)
{
    Std_ReturnType ret = (Std_ReturnType)0x01;
    if(port > 5 -1)
